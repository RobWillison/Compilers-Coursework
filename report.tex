...
nested function definitions. In the TAC these nested functions are flatterend so
there are no nested function definitions, this was done to simplify and unify the way
all functions apear in the generated mips code. For this to work the TAC translater renames
...
The environment for the TAC compiler consitits of a linked list of frames each frame
contating a binding of token to the register it is contained in this is one of the
simplest to implement an enviroment \cite{Cardelli:1984:CFL:800055.802037}. Another
reason for this enviroment structure is it means all a closure object needs is a
pointer to the coresponding frame and the function start address.  
